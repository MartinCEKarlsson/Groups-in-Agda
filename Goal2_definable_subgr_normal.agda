...
center G = record { prop = λ g → in-center g ; f = in-center-is-prop ; id = unit-in-center
  ...
  where
    ...
    {- We need to show that the center is closed under multiplication and inverses,
...
{- In this example we will define the commutator subgroup and use our main result to show it is normal.
   This is a generated subgroup, so we will need to introduce some more machinery in order
   to define it. We will 'build it up from below' by taking a generating set and closing this 
   artificially under everything that we need. We do so by using 'words' -}
...
{- We model a (generating) subset of a group as a function of some type into the group.
   The subset is then intuitively its image -}
subset : {ℓ : ULevel} (G : Group {ℓ}) → (A : Type ℓ) → Type ℓ
subset G A = A → Group.U G

{- We build up words syntactically, articifically closing it under the right operations -}
data word {ℓ : ULevel} (A : Type ℓ) : Type ℓ  where
  empt : word A --the empty word
  inst : A → word A --elements of A themselves are words
  mult : word A → word A → word A --words are closed under multiplication 
  inv : word A → word A --words are closed under inverses

module _  {G : Group {α}} {A : Type α} where
  open Group G
    
  {- From a subset of G and a word of A, we can compute the corresponding element in G, 
     which corresponds exactly to how we set up the 'word A' datatype. 
     In a sense, this is the 'interpretation' of our datatype word A into G -}
  compute-word : subset G A → (word A → U)
  compute-word f empt = e
  compute-word f (inst x) = f x
  compute-word f (mult wrd wrd₁) = (compute-word f wrd) · (compute-word f wrd₁)
  compute-word f (inv wrd) = i (compute-word f wrd)

  {- We can now define a generated subgroup of G. Everything works smoothly since we closed the subgroup 
     artificially under the necessary operations. To work with the truncation more efficiently we
     use do-notation -}
  gen-subgroup : subset G A → Subgrp G
  gen-subgroup f' = record { prop = prop ; f = Trunc-level ; id = [ empt , idp ] ; comp = comp-pres
                           ; inv = inv-pres }
    where
      {- the prop of g ∈ G requires the existence of a word that maps to the element g under compute-word, 
         So we capture exactly the image of word A under the compute-word map -}
      prop : U → Type α
      prop g =  ∥ Σ (word A) (λ φ → (compute-word f' φ) == g) ∥ 
      --we use the truncation here to make sure it is a proposition
  
      {- if there is a word u that maps to a and a word v that maps to b, then 'mult u v' maps 
         to (a · b) by definition -}
      comp-pres : {a b : U} → prop a → prop b → prop (comp a b)
      comp-pres {a} {b} aprop bprop =
       do 
         (u , p) ← aprop
         (v , q) ← bprop 
         return ((mult u v) , (
           compute-word f' (mult u v) =⟨ idp ⟩
           compute-word f' u · compute-word f' v 
           =⟨ comp-is-congr (compute-word f' u) a (compute-word f' v) b p q ⟩
           a · b =∎))

      {- if there is a word u that maps to a, then 'inv u' maps to (i a) by definition -}
      inv-pres : {a : U} → prop a → prop (i a)
      inv-pres {a} aprop = 
        do 
          (u , p) ← aprop 
          return ((inv u) , (
            compute-word f' (inv u) =⟨ idp ⟩
            i (compute-word f' u) =⟨ ap (λ φ → i φ) p ⟩
            (i a) =∎))

{- We define a pair of group elts as a pair from their underlying sets -}   
record Pair-Group {ℓ : ULevel} (A B : Group {ℓ}) : Set ℓ where
  constructor GPair
  field
    g : Group.U A
    h : Group.U B

{- Now we can define the commutator subgroup, 
   the subgroup generated by all the commutators [g,h] = g · h · ig · ih -}
commutator-map : {G : Group {α}} → Pair-Group G G → Group.U G
commutator-map {G} pair = g · ( h · ( (i g) · (i h) ) )
    where
      open Pair-Group pair
      open Group G

commutatorSubgrp : (G : Group {α}) → Subgrp G
commutatorSubgrp G = gen-subgroup {G} {Pair-Group G G} commutator-map

{- And we can prove that the commutator subgroup is normal by our main result -}
commutatorSubgrp-is-normal : {G : Group {α}} → is-normal (commutatorSubgrp G)
commutatorSubgrp-is-normal {G} = def-subgroups-are-normal commutatorSubgrp G

{- Again, for contrast we can also give a direct proof that the commutator subgroup [G,G] is normal -}
{- for any h ∈ [G,G], some word u maps to h. Then the word -mult (inst (f,g)) u- 
   maps to  [f,g] · h =  g · h · (i g). The proof is mainly applying associativity -}
commutatorSubgrp-is-normal' : {G : Group {α}} → is-normal (commutatorSubgrp G)
commutatorSubgrp-is-normal' {G} = λ g h hprop → 
  do
    (u , p) ← hprop
    return ( (mult (inst (GPair g h)) u) , (
      (g · (h · ( (i g) · (i h)))) · (compute-word commutator-map u)
               =⟨ ap (λ φ → (g · (h · ((i g) · (i h)))) · φ) p ⟩
      (g · (h · ( (i g) · (i h)))) · h 
               =⟨ associative g (h · ( (i g) · (i h))) h ⟩
      g · ((h · ( (i g) · (i h))) · h)
               =⟨ ap (λ φ → g · φ) (associative h ((i g) · (i h)) h) ⟩
      g · (h · (( (i g) · (i h)) · h)) 
               =⟨ ap (λ φ → g · (h · φ)) (associative (i g) (i h) h) ⟩
      g · (h · ( (i g) · ((i h) · h))) 
               =⟨ ap (λ φ → g · (h · ( (i g) · φ))) (inv-l h) ⟩
      g · (h · ( (i g) · e)) =⟨ ap (λ φ → g · (h · φ)) (unit-r (i g)) ⟩
      g · (h · (i g)) =∎) )
    where 
      open Group G
