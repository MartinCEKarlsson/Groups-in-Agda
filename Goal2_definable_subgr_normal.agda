...
{- We define a pair of groups as a pair of their underlying sets -}   
record Pair {ℓ : ULevel} (A B : Group {ℓ}) : Set ℓ where
  field
    g : Group.U A
    h : Group.U B
...
commutator-map : {G : Group {α}} → Pair G G → Group.U G
...
commutatorSubgrp G = gen-subgroup {G} {Pair G G} commutator-map

{- And finally we can prove that the commutator subgroup is normal by our main result -}
